...
export default function BuyNowCheckoutPage() {
  ...
  // product detail page fetches the full product via its own API call using
  // this slug/id — clicking the summary here just routes there
  const productHref = item ? `/products/${item.documentId || item.id}` : null;
  ...
  return (
    <div className="page-wrapper mx-auto px-4 md:px-0 py-6 md:py-10 pb-32">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl md:text-2xl font-semibold text-gray-900">
          Checkout
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
        {/* Left — compact product summary: small image, minimal details.
            Clickable through to the product detail page, which does its own
            fetch against the product API for the full listing. */}
        <div>
          <Link
            href={productHref}
            className="flex items-start gap-4 bg-white rounded-2xl border border-gray-100 p-4 hover:border-gray-200 transition-colors"
          >
            <div className="w-16 h-16 flex-shrink-0 rounded-xl bg-gray-50 flex items-center justify-center overflow-hidden">
              <img
                src={item.image || "/placeholder.jpg"}
        ...
        {/* Right — address, payment, price breakdown (desktop). No button
            here anymore — bottom bar owns the Place Order action everywhere. */}
        <div className="space-y-4">
          <div className="sticky top-24 space-y-4">
            <AddressPicker
              addresses={addresses}
      ...
      {/* Bottom fixed bar — amount + Place Order, both right-aligned */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 md:px-8 py-3 flex   z-40 ">
        <div className="flex gap-4 md:gap-10 page-wrapper items-center justify-end">
          ...
          <button
            onClick={handlePlaceOrder}
            disabled={placing}
            className="max-w-[220px] py-3 px-6 rounded-lg bg-cyan-900 text-white text-sm font-semibold hover:bg-cyan-800 transition-colors disabled:opacity-60"
          >
            {placing ? "Placing..." : "Place Order"}
          </button>
        </div>
        {proceedError && (
          <p className="fixed bottom-[76px] left-0 right-0 text-center text-sm text-red-500 px-4">
            {proceedError}
          </p>
        )}
      </div>
