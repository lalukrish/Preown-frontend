...
export async function generateMetadata() {
    ...
    return generateMetadataFromSEO(seoData, 'Blog - Preown by applebae', 'Welcome to Preown by applebae Blog. Discover the latest news, tips, and insights about premium phones and gadgets.', '/blog');
}
        ...
        <>
            ...
            <div className={styles.blogPage}>
                <section className={styles.hero}>
                    <div className={styles.container}>
                        <h1 className={styles.title}>
                            Welcome to <span className={styles.highlight}>Preown by applebae</span> Blog
                        </h1>
                        ...
                        {blogs.length === 0 ? (
                            ...
                            <div className={styles.postsGrid}>
                                {blogs.map((item) => {
                                    ...
                                    const imageUrl = imagePath ? `${STRAPI_IMAGE_BASE_URL}${imagePath}` : '/next.svg';
                                    const title = item?.heading || 'Untitled';
                                    const date = item?.date || item?.createdAt;
                                    // Fix: use discription (with 'i') instead of description
                                    const excerpt = item?.subHeading || getFirstParagraphText(item?.discription || item?.description) || '';
                                    ...
                                    return (
                                        <article key={item.id || item.documentId} className={styles.postCard}>
                                            <Link href={`/blog/${urlSlug}`} className={styles.cardLink}>
                                                ...
                                                <div className={styles.cardContent}>
                                                    <div className={styles.postHeader}>
                                                        <span className={styles.category}>{item?.category || "Blog"}</span>
                                                        <span className={styles.date}>{date ? new Date(date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : ''}</span>
                                                    </div>
                ...
                <section className={styles.ctaSection}>
                    <div className={styles.container}>
                        <h2 className={styles.ctaTitle}>Ready to Explore Our Collection?</h2>
                        <p className={styles.ctaDescription}>
                            Browse our premium selection of pre-owned phones and latest gadgets today.
                        </p>
                        <div className={styles.ctaButtons}>
                            <a href="/products" className={styles.buttonFilled}>Shop Now</a>
                            <a href="/about" className={styles.buttonOutlined}>Learn More</a>
                        </div>
                    </div>
                </section>
            </div>
