...
import styles from "./PhoneSelectorSection.module.css";
...
const PhoneSelectorSection = ({ featured }) => {
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div className={styles.cardGrid} ref={scrollContainerRef}>
          {phones.slice(0, displayCount).map((phone, index) => {
            const images = phone.ProductImagesAndVideos;
            const imageUrl =
              images && images[0]
                ? `https://backapp.preown.store${images[0].formats?.small?.url || images[0].url}`
                : "/placeholder.jpg";

            const productSlug = phone?.documentId || phone?.id;
...
export default PhoneSelectorSection;
